...
> **Версія:** 1.4 · **Створено:** 2026-07-06 · **Оновлено:** 2026-07-09
...
| Код | Вимога |
|---|---|
| **FR-ATTACH-01** | До заявки можна прикріпити **зображення** (JPEG/PNG/WebP): ≤ 10 МБ на файл, ≤ 10 файлів на заявку. Інші типи (зокрема HEIC — Р-13) та розміри відхиляються зі зрозумілою помилкою. |
...
- **otp_code** — транзитний стор OTP/rate-limit (ADR-0004): `phone`, `code_hash`,
  `expires_at`, `attempts`, `consumed_at?`, `created_at`. Ліміти надсилання
  (60 с / доба, FR-AUTH-03) — **похідні** від рядків таблиці (вона ж лог
  надсилань за останню добу), окремих стовпців-лічильників немає (Р-12).
- **session** — server-side сесія (ADR-0004, NFR-SEC-01): `id`, `token_hash`
  (unique; у cookie — випадковий токен, у БД — лише його SHA-256-хеш),
  `user_id`, `expires_at` (≥30 днів, FR-AUTH-04), `created_at` (Р-12).
...
erDiagram
    ...
    user ||--o{ session : "сесії (httpOnly cookie)"
...
| **HEIC з iPhone** не відображається в браузерах | Низький | ✅ Вирішено (Р-13, v1.4): прийнято лише JPEG/PNG/WebP; iPhone Safari конвертує HEIC→JPEG при аплоаді через `accept`-фільтр |
...
| 2026-07-08 | **v1.3:** §9.1 приведено у відповідність до реалізації S-02 (Р-12): додано сутність `session` (server-side сесія, ADR-0004); уточнено `otp_code` — ліміти надсилання похідні від рядків, не стовпці-лічильники. Зміст вимог (FR/NFR) без змін. |
| 2026-07-09 | **v1.4:** FR-ATTACH-01 звужено — типи вкладень лише JPEG/PNG/WebP, HEIC відхиляється (Р-13, закриває В-02); анотовано ризик §11. Рішення користувача на старті S-07. |
