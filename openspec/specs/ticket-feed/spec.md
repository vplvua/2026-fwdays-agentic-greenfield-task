# ticket-feed Specification

## Purpose

The single append-only chronological feed per ticket (PRD §5.5,
FR-FEED-01/02, FR-TICKET-03, FR-DUE-01, FR-ACCESS-01): user text notes and
automatic system events — status changes and tracked field changes with
old → new value snapshots — in one owner-scoped feed, visually distinct in
the SPA, with no editing or deleting of items.
