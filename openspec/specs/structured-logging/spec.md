...
Structured, PII-free API logging (NFR-OBS-01, logs part — the health part
lives in `app-skeleton`; log-content constraints per NFR-SEC-01): one
structured entry per `/api/*` request (single-line JSON in production,
compact human-readable line in development), unhandled server errors with
stack traces, OTP SMS send outcomes with masked phones, and no PII or
secrets in any log entry.
